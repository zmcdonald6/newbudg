...
if not st.session_state.authenticated:
    ...
    with st.form("login_form"):
        email = st.text_input("Email (case sesnitive)", key="login_email")
            ...
            if user:
                try:
                    ...
                    if bcrypt.checkpw(password.encode(), decoded_hash):
                        ...
                        st.session_state.user_record = user  # Save the full user data
        ...
        with st.form("change_password"):
            ...
            if submit_change:
                if new_password != confirm_password:
                    ...
                    row = user["row_index"]
                    sheet.update_cell(row, 4, encoded)  # hashed_password
                    sheet.update_cell(row, 6, "FALSE")  # first_login
